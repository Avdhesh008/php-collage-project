    ...
    <table class="table table-bordered">      
      ...
      <tbody id='sdfsdfdsfs'>
        ...
        <tr>
          ...
          <td class="flex">
            @if($val->status==0)
            <a href="{{url('admin/employee-block',['id'=>encrypt($val->id),'status'=>1])}}"><button class="btn btn-success">Block</button></a>
            @else
            <a href="{{url('admin/employee-block',['id'=>encrypt($val->id),'status'=>0])}}"><button class="btn btn-danger">UnBlock</button></a>

            @endif
            <form action="{{url('admin/delete-employee',['id'=>encrypt($val->id)])}}" method="POST" id="deleteuser">
              @csrf

              <a href="javascript:void(0);" onclick="submitForm()"><button class="btn btn-danger">Delete</button></a>
            </form>
            <a href="{{url('admin/edit_employee/'.$val->id)}}"><button type="submit" class="btn btn-info">Edit</button></a>
          </td>
        </tr>
